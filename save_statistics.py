...
def save_plays_log(playerlog):
	filename = './historico/dados_jogadas_nn.csv'
	columns, data = get_plays_csv_structure(playerlog)
	check_folder_exists('./historico')
	file_exists = os.path.isfile('./historico/dados_jogadas_nn.csv')
...
def save_match_log(playerlog):
	filename = './historico/dados_partidas_nn.csv'
	columns, data = get_match_csv_structure(playerlog)
	file_exists = os.path.isfile('./historico/dados_partidas_nn.csv')
	check_folder_exists('./historico')
	with open(filename, 'a+') as csvfile:
		writer = csv.DictWriter(csvfile, fieldnames=columns)
		if not file_exists:
